...
`vercel.json`, `.vercelignore`, and `api/index.py` are already set up. On
this account, Vercel's importer treats the project as "other framework"
rather than auto-detecting FastAPI, which means it only looks for
Serverless Functions inside an `api/` directory — a root-level `main.py`
alone isn't picked up as a function. `api/index.py` is a thin wrapper that
just imports the real `app` from `main.py` (no logic is duplicated); it's
what makes the project deployable without restructuring the rest of the
code. If you deploy under a different account/flow where Vercel *does*
auto-detect FastAPI from a root `main.py`, `api/index.py` is harmless
either way — it just won't be the one Vercel picks.

Use **vercel.com/new**, not **v0.app** — v0 is Vercel's separate AI
app-builder product with its own credit-based billing; it is not the same
as importing an existing repo into normal Vercel hosting, and will try to
provision its own AI credentials/credits if you go through it by mistake.
